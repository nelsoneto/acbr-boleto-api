...
class ACBrBoleto {
    ...
    constructor() {
        ...
        try {
            ...
            this.Boleto_Inicializar = this.lib.func(convention, 'Boleto_Inicializar', 'int', [koffi.out('uintptr_t *'), 'string', 'string']);
            this.Boleto_Finalizar = this.lib.func(convention, 'Boleto_Finalizar', 'int', ['uintptr_t']);
            this.Boleto_ConfigGravarValor = this.lib.func(convention, 'Boleto_ConfigGravarValor', 'int', ['uintptr_t', 'string', 'string', 'string']);
            this.Boleto_IncluirTitulos = this.lib.func(convention, 'Boleto_IncluirTitulos', 'int', ['uintptr_t', 'string', 'string']);
            this.Boleto_GerarPDF = this.lib.func(convention, 'Boleto_GerarPDF', 'int', ['uintptr_t']);
            this.Boleto_LimparLista = this.lib.func(convention, 'Boleto_LimparLista', 'int', ['uintptr_t']);
        } catch (e) {
    ...
    inicializar(eArqConfig: string = '', eChaveCrypt: string = '') {
        let outHandle = [BigInt(0)];
        const res = this.Boleto_Inicializar(outHandle, eArqConfig, eChaveCrypt);
        if (res === 0) {
